...
os.makedirs("split_datasets", exist_ok=True)  # New directory for split datasets
os.makedirs("split_datasets/csv", exist_ok=True)  # CSV format
os.makedirs("split_datasets/npy", exist_ok=True)  # NumPy format

# Initialize results storage
full_results = []

def save_split_datasets(dataset_name, X_train, X_test, y_train, y_test):
    """Save the split datasets in multiple formats for later use"""
    
    # Save as CSV files (human-readable, widely compatible)
    ...
    y_train_df = pd.DataFrame(y_train, columns=['target']) if not isinstance(y_train, pd.DataFrame) else y_train
    y_test_df = pd.DataFrame(y_test, columns=['target']) if not isinstance(y_test, pd.DataFrame) else y_test
    
    # Save CSV files
    X_train_df.to_csv(f"{csv_dir}/X_train.csv", index=False)
    X_test_df.to_csv(f"{csv_dir}/X_test.csv", index=False)
    y_train_df.to_csv(f"{csv_dir}/y_train.csv", index=False)
    y_test_df.to_csv(f"{csv_dir}/y_test.csv", index=False)
    
    # Save as NumPy arrays (efficient storage, fast loading)
    ...
    metadata = {
        ...
        'train_class_distribution': dict(pd.Series(y_train).value_counts().sort_index()),
        'test_class_distribution': dict(pd.Series(y_test).value_counts().sort_index()),
        'random_state': SEED
    }
    
    # Save metadata as JSON-like format
    import json
    with open(f"{csv_dir}/metadata.json", 'w') as f:
        json.dump(metadata, f, indent=2)
    
    with open(f"{npy_dir}/metadata.json", 'w') as f:
        json.dump(metadata, f, indent=2)
    
    print(f"   💾 Saved split datasets in CSV and NumPy formats")
    print(f"      CSV: {csv_dir}")
    print(f"      NumPy: {npy_dir}")

def load_split_datasets_example(dataset_name, format='csv'):
    """
    Example function showing how to load the saved datasets later
    ...
    if format == 'csv':
        base_dir = f"split_datasets/csv/{dataset_name}"
        ...
        base_dir = f"split_datasets/npy/{dataset_name}"
        ...
        raise ValueError("Format must be 'csv' or 'npy'")
...
for dataset_name, config in DATASETS.items():
    try:
        ...
        save_split_datasets(dataset_name, X_train, X_test, y_train, y_test)
...
print("="*50)
print("Split datasets saved in two formats:")
print("📁 CSV format: split_datasets/csv/{dataset_name}/")
print("📁 NumPy format: split_datasets/npy/{dataset_name}/")
print("\nEach folder contains:")
print("  - X_train.csv/npy (training features)")
print("  - X_test.csv/npy (test features)")
print("  - y_train.csv/npy (training labels)")
print("  - y_test.csv/npy (test labels)")
print("  - metadata.json (dataset information)")
print("\nEnhanced processing complete!")

# Example of how to load the datasets later
print("\n" + "="*50)
print("EXAMPLE: How to load saved datasets later")
print("="*50)
print("""
# Load CSV format (human-readable, widely compatible)
X_train, X_test, y_train, y_test, metadata = load_split_datasets_example('heart', format='csv')

# Load NumPy format (fast loading, efficient storage)
X_train, X_test, y_train, y_test, metadata = load_split_datasets_example('heart', format='npy')

# Or load manually:
# CSV:
import pandas as pd
X_train = pd.read_csv('split_datasets/csv/heart/X_train.csv')
y_train = pd.read_csv('split_datasets/csv/heart/y_train.csv')['target']

# NumPy:
import numpy as np
X_train = np.load('split_datasets/npy/heart/X_train.npy')
y_train = np.load('split_datasets/npy/heart/y_train.npy')
""")