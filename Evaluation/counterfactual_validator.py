...
#     validate_multiple_reports("/", "./validation_results/")
...
# Define your datasets configuration to match the generator
ONLINE_DATASETS = {
    "GermanCredit": {
...
class CounterfactualValidator:
    def __init__(self, dataset_name=None, model_name=None):
        ...
        self.data = None
        self.model = None
        self.encoder = None  # For class label handling
        self.feature_frequency = {}  # Track feature usage frequency
        self.feature_stats = {}  # Store feature statistics for adaptive scaling
    ...
    def parse_filename(self, filename):
        ...
        # Try to find dataset name (including multi-word datasets)
        for dataset_key in ALL_DATASETS.keys():
            # Handle spaces in dataset names
            dataset_key_parts = dataset_key.replace(' ', '_').split('_')
            if all(part in parts for part in dataset_key_parts):
                detected_dataset = dataset_key
                break
            # Also try lowercase matching
            if dataset_key.lower().replace(' ', '_') in '_'.join(parts).lower():
                detected_dataset = dataset_key
                break
        ...
        if not detected_dataset or not detected_model:
            ...
            for dataset_key in ALL_DATASETS.keys():
                for model_key in ML_MODELS.keys():
                    expected_pattern = f"{dataset_key.replace(' ', '_')}_{model_key}"
                    if expected_pattern.lower() in basename.lower():
                        detected_dataset = dataset_key
                        detected_model = model_key
                        break
                if detected_dataset and detected_model:
    ...
    def setup_from_filename(self, counterfactual_csv_path):
        ...
        if dataset_name in ONLINE_DATASETS:
            self.target_col = ONLINE_DATASETS[dataset_name]['target_col']
        elif dataset_name in SKLEARN_DATASETS:
            self.target_col = SKLEARN_DATASETS[dataset_name]['target_col']
        else:
            self.target_col = 'target'  # fallback
    ...
    def load_data(self):
        ...
        try:
            # Try to load from CSV files (same structure as generator)
            ...
            if os.path.exists(train_path) and os.path.exists(test_path):
                ...
                print(f"Dataset loaded from CSV files: {self.data.shape}")
                
            else:
                # If CSV files don't exist, try to load from sklearn datasets
                if self.dataset_name in SKLEARN_DATASETS:
                    loader = SKLEARN_DATASETS[self.dataset_name]['loader']
                    sklearn_data = loader()
                    
                    # Create DataFrame
                    feature_df = pd.DataFrame(sklearn_data.data, columns=sklearn_data.feature_names)
                    feature_df[self.target_col] = sklearn_data.target
                    self.data = feature_df
                    
                    print(f"Dataset loaded from sklearn: {self.data.shape}")
                else:
                    raise FileNotFoundError(f"Cannot find data files for {self.dataset_name}")
    ...
    def load_model(self):
        ...
        try:
            ...
            if model_path is None:
                raise FileNotFoundError(f"Model file not found. Tried: {possible_paths[:3]}...")
            
            # Load model and encoder if available
            loaded = joblib.load(model_path)
            
            if isinstance(loaded, tuple) and len(loaded) == 2:
                self.model, self.encoder = loaded
                print(f"✓ Loaded model and encoder from {model_path}")
            else:
                self.model = loaded
    ...
    def parse_counterfactual_rules(self, rules_str):
        ...
        if 'No rules extracted' in rules_str or 'Error extracting' in rules_str:
            return conditions
        ...
        for part in rule_parts:
            ...
            try:
                ...
                print(f"Warning: Could not parse rule part '{part}': {e}")
                continue
    ...
    def predict_with_model(self, sample_data):
        ...
        # Prepare feature data (exclude target if present)
        if isinstance(sample_data, pd.Series):
            feature_cols = [col for col in sample_data.index if col != self.target_col]
            features = sample_data[feature_cols].values.reshape(1, -1)
        else:
            # If it's already a DataFrame or array
            features = sample_data.reshape(1, -1) if len(sample_data.shape) == 1 else sample_data
        ...
        try:
            ...
            # Debug output
            print(f"Raw prediction: {prediction}, Probabilities: {prob}")
    ...
    def map_prediction_to_class(self, prediction_value):
        """Map prediction value to class label using available mappings"""
        # 1. Try encoder first
        if self.encoder is not None:
            try:
                return self.encoder.inverse_transform([prediction_value])[0]
            except Exception as e:
                print(f"Encoder error: {e}")
        
        # 2. Try model's classes_ attribute
        if hasattr(self.model, 'classes_'):
            try:
                return self.model.classes_[prediction_value]
            except IndexError:
                print(f"Index error with prediction value {prediction_value} for classes {self.model.classes_}")
    ...
    def validate_counterfactual_sample(self, instance_id, actual_class, predicted_class, 
        ...
        if original_prediction is not None and modified_prediction is not None:
            ...
            # Map predictions to class labels using unified method
            orig_class = self.map_prediction_to_class(orig_pred)
            mod_class = self.map_prediction_to_class(mod_pred)
            ...
            # Debug output
            print(f"Prediction mapping debug:")
            print(f"  Raw original prediction: {orig_pred} -> Mapped: {orig_class}")
            print(f"  Raw modified prediction: {mod_pred} -> Mapped: {mod_class}")
