...
def load_preprocessed_dataset(dataset_name):
    ...
    base_path = f"preprocessed_data/{dataset_name}"
...
class EpochBasedXGBClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, n_estimators=200, learning_rate=0.1, max_depth=3, 
                 early_stopping_rounds=15, validation_fraction=0.2, 
                 random_state=None, verbose=False, **xgb_params):
        self.n_estimators = n_estimators
        self.learning_rate = learning_rate
        self.max_depth = max_depth
        self.early_stopping_rounds = early_stopping_rounds
        ...
        self.random_state = random_state
        self.verbose = verbose
        self.xgb_params = xgb_params  # Store XGBoost-specific parameters
    ...
    def fit(self, X, y):
        ...
        # Pass all XGBoost parameters to the classifier
        self.xgb_ = XGBClassifier(
            n_estimators=self.n_estimators,
            learning_rate=self.learning_rate,
            max_depth=self.max_depth,
            random_state=self.random_state,
            **self.xgb_params  # Forward all additional parameters
        )
        ...
        self.evals_result_ = self.xgb_.evals_result_
        self.classes_ = self.xgb_.classes_
        return self
...
def build_robust_model_pipelines(n_features):
    """Create robust model pipelines without preprocessing (data is already preprocessed)"""
    ...
    models = {
        ...
        'XGBoost': Pipeline([
            ('classifier', EpochBasedXGBClassifier(
                ...
                # XGBoost-specific params now go here:
                subsample=0.8,
                colsample_bytree=0.8,
                reg_alpha=0.1,
                reg_lambda=10,
...
for dataset_name in DATASETS.keys():
    try:
        ...
        X_train, X_val, X_test, y_train, y_val, y_test, metadata = load_preprocessed_dataset(dataset_name)
        ...
        # Build robust models
        models = build_robust_model_pipelines(n_features)
        param_grids = get_conservative_param_grids()
        dataset_results = []
        ...
        for model_name, pipeline in models.items():
            ...
            try:
                ...
                # Generate and save training plots for epoch-based models
                if hasattr(best_model.named_steps.get('classifier', None), 'plot_training_history'):
                    try:
                        plot_path = f"validation_plots/{dataset_name}_{model_name.replace(' ', '_')}_training_history.png"
                        fig = best_model.named_steps['classifier'].plot_training_history(save_path=plot_path)
                        if fig:
