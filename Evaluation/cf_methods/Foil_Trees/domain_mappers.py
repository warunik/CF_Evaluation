...
class DomainMapperTabular(DomainMapper):
    ...
    def explain(self,
                ...
                **kwargs):
        """
        Returns an explanation as a structured dictionary
        ...
        explanation = {
            "Predicted Class": fact_name,
            "Contrast Class": foil_name,
            "Counterfactuals": self.rule_to_str(counterfactuals) or "N/A",
            "Confidence": round(confidence, 4),
            "Fidelity": round(fidelity, 4),
            "Time Taken (s)": round(time, 2)
        }

        # If factual rules are provided, add additional explanation
        if factuals is not None:
            explanation["Additional Factual Explanation"] = self.rule_to_str(factuals, remove_last=True)
