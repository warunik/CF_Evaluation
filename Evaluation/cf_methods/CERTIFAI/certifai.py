...
# Example: How to use CERTIFAI to generate counterfactuals
...
# Convert to DataFrame for CERTIFAI - use integer column names to avoid KeyError
...
# 3. INITIALIZE CERTIFAI
# Option 1: Initialize with pandas DataFrame
certifai = CERTIFAI(Pm=0.2, Pc=0.5, numpy_dataset=df_train)

# Option 2: Initialize from CSV file
# certifai = CERTIFAI.from_csv('path/to/your/data.csv')

# 4. GENERATE COUNTERFACTUALS
certifai.fit(
    model=model,                    # Your trained model
    x=df_test,                     # Test data to generate counterfactuals for
    pytorch=True,                   # Set to True for PyTorch models
    classification=True,            # Set to False for regression
    generations=3,                  # Number of genetic algorithm generations
    distance='automatic',           # Distance metric ('automatic', 'L1', 'L2', 'SSIM')
    constrained=True,              # Constrain features to training data range
    select_retain=1000,            # Number of candidates to retain after selection
    gen_retain=500,                # Number to retain per generation
    final_k=1,                     # Number of final counterfactuals per sample
    normalisation=None,            # 'standard', 'max_scaler', or None
    verbose=True                   # Show progress bar
)

# 5. ACCESS RESULTS
# Results are stored in certifai.results
# Each result is a tuple: (original_sample, counterfactuals, distances)
for i, (original, counterfactuals, distances) in enumerate(certifai.results[:3]):
    print(f"\nSample {i}:")
    print(f"Original sample shape: {original.shape}")
    print(f"Number of counterfactuals: {len(counterfactuals)}")
    print(f"Distance to counterfactual: {distances[0]:.4f}")
    
    # Convert back to DataFrame for easier viewing
    original_df = pd.DataFrame(original, columns=df_test.columns)
    counterfactual_df = pd.DataFrame([counterfactuals[0]], columns=df_test.columns)
    
    print("Original vs Counterfactual:")
    comparison = pd.concat([original_df, counterfactual_df], 
                          keys=['Original', 'Counterfactual'])
    print(comparison)

# 6. EVALUATE MODEL ROBUSTNESS
# Calculate CER (Counterfactual-based Explanation for Robustness) score
cer_score = certifai.check_robustness(x=df_test, normalised=False)
print(f"\nCER Score (higher = more robust): {cer_score:.4f}")

# Calculate normalized CER score
ncer_score = certifai.check_robustness(x=df_test, normalised=True)
print(f"Normalized CER Score: {ncer_score:.4f}")

# 7. ANALYZE FEATURE IMPORTANCE
# Check which features are most important for generating counterfactuals
# FIX: Always pass the x parameter to avoid the UnboundLocalError
feature_importance = certifai.check_feature_importance(
    x=df_test,  # IMPORTANT: Always provide the x parameter
    sensibility=10,
    print_results=True,
    visualise_results=True
)

# 8. CHECK FAIRNESS (if you have categorical features)
# Example: Check fairness across different subgroups
# This requires categorical features in your dataset
"""
fairness_results = certifai.check_fairness(
    control_groups=[
        {'categorical_feature': 'group_A'},
        {'categorical_feature': 'group_B'}
    ],
    x=df_test,
    print_results=True,
    visualise_results=True
...
# Fix certain features during counterfactual generation
certifai_fixed = CERTIFAI(numpy_dataset=df_train)
certifai_fixed.fit(
    model=model,
    x=df_test,
    pytorch=True,
    classification=True,
    fixed=[0, 1],  # Keep these feature indices unchanged (use integers, not strings)
    generations=3,
    verbose=True
)

# Generate counterfactuals for specific class predictions only
certifai_class_specific = CERTIFAI(numpy_dataset=df_train)
certifai_class_specific.fit(
    model=model,
    x=df_test,
    pytorch=True,
    classification=True,
    class_specific=1,  # Only generate for samples predicted as class 1
    generations=3,
    verbose=True
)
