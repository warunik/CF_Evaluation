...
class AutomatedFeasibilityAnalyzer:
    def __init__(self):
        # Updated with more precise realistic ranges based on your datasets
        self.feature_constraints = {
            'heart': {
                'immutable': ['age', 'sex'],
                'partially_immutable': {
                ...
                'mutable': ['trestbps', 'chol', 'fbs', 'thalach', 'exang', 'oldpeak'],
                'realistic_ranges': {
                    'age': (29, 77),          # Actual range in your sample
                    'trestbps': (94, 200),    # Resting blood pressure (94-200 mmHg)
                    'chol': (126, 564),       # Serum cholesterol (126-564 mg/dl)
                    'thalach': (71, 202),     # Maximum heart rate (71-202 bpm)
                    'oldpeak': (0, 6.2),      # ST depression (0-6.2)
                    'fbs': (0, 1),            # Fasting blood sugar (0-1)
                    'exang': (0, 1),          # Exercise induced angina (0-1)
                    'sex': (0, 1),            # Sex (0-1)
            ...
            'diabetes': {
                'immutable': ['age'],
                'partially_immutable': {
                    'pregnancies': (0, 17),   # Based on your sample max
                    'age': (21, 81)           # Actual range in your sample
                },
                'mutable': ['glucose', 'blood_pressure', 'skin_thickness', 'insulin', 'bmi', 'diabetes_pedigree'],
                'realistic_ranges': {
                    'pregnancies': (0, 17),
                    'glucose': (0, 199),          # Blood glucose (0-199 mg/dL)
                    'blood_pressure': (0, 122),   # Diastolic blood pressure (0-122 mmHg)
                    'skin_thickness': (0, 99),    # Triceps skin fold thickness (0-99 mm)
                    'insulin': (0, 846),          # 2-Hour serum insulin (0-846 mu U/ml)
                    'bmi': (0, 67.1),             # Body mass index (0-67.1)
                    'diabetes_pedigree': (0.08, 2.42),  # Actual range in your sample
            ...
            'german': {
                ...
                'partially_immutable': {
                    'age': (19, 75),          # Actual range in your sample
                    'duration': (4, 72),      # Credit duration in months
                    'credit_amount': (250, 18424),  # Actual range in your sample
                    'residence_since': (1, 4) # Years at current residence
                },
                'mutable': ['checking_status', 'credit_history', 'purpose', 'savings',
                ...
                'realistic_ranges': {
                    ...
                    'num_dependents': (1, 2)  # Based on your sample
                }
            }
        }
    ...
    def parse_counterfactual_rules(self, rules_str: str) -> List[Tuple[str, str, float]]:
        ...
        # Handle different formats: both "feature operator value" and "feature: operator value"
        rules_str = re.sub(r'(\w+):\s*([<>=!]+)\s*(-?\d+\.?\d*)', r'\1 \2 \3', rules_str)
        
        # Split by 'and' and clean up
        parts = [p.strip() for p in rules_str.split(' and ') if p.strip()]
        ...
        for part in parts:
            ...
            if match:
                ...
                # Try to match without spaces around operator
    ...
    def check_feature_feasibility(self, dataset: str, feature: str, operator: str, 
        ...
        if feature in constraints.get('partially_immutable', {}):
            ...
            if isinstance(constraint, tuple):  # Range constraint
                ...
                if target_value not in constraint:
                    result['feasible'] = False
                    result['reason'] = f"{feature} target value {target_value} not in allowed categories {constraint}"
                    return result
    ...
    def analyze_instance_feasibility(self, instance_id: Any, counterfactual_rules: str, 
        ...
        # Get original instance data
        if instance_id in original_data.index:
            original_instance = original_data.loc[instance_id]
        else:
            # Try integer index if instance_id is integer-like
            try:
                idx = int(instance_id)
                if idx in original_data.index:
                    original_instance = original_data.loc[idx]
                else:
                    return {
                        'instance_id': instance_id,
                        'feasible': False,
        ...
        original_values = original_instance.to_dict()
...
def batch_analyze_datasets(base_dir: str = ".", output_dir: str = "feasibility_reports"):
    ...
    datasets = ['HeartDisease', 'Diabetes', 'adult', 'GermanCredit', 'iris', 'wine', 'breastcancer']
    ...
    for dataset in datasets:
        ...
        for model in models:
            ...
            # Updated file paths
            cf_file = os.path.join(base_dir, f"{dataset}_{model}_counterfactual_report.csv")
            val_file = os.path.join(base_dir, "validation_results", f"{dataset}_{model}_validation_summary.csv")
            orig_file = os.path.join(base_dir, "data", f"{dataset}.csv")
...
def main():
    ...
    if args.dataset and args.model:
        # Analyze specific dataset-model combination
        cf_file = os.path.join(args.base_dir, f"{args.dataset}_{args.model}_counterfactual_report.csv")
        val_file = os.path.join(args.base_dir, "validation_results", f"{args.dataset}_{args.model}_validation_summary.csv")
        orig_file = os.path.join(args.base_dir, "data", f"{args.dataset}.csv")
